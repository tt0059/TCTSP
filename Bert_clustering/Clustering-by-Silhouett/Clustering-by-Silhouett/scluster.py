    ...
    def fit(self,data):
        ...
        for i in range(self.org, self.lim , self.stp):
            ...
            if ((self.max - self.min_unclu_ratio) < (silho - unclu_ratio)): #self.max < silho or ((self.max - silho) <1e-3 and (self.min_unclu_ratio - unclu_ratio)>1e-3):
                ...
                np.save('probs_norm_tp.npy', probs)                
            ...
            if(renew == False):
                ...
                print(f'\nthe current best choice is min topic size to {min_topic_size}, takes {time.time()-st}s, cluster_num = {num_cluster}, with {round(self.max,4)} silhouette score and {round(self.min_unclu_ratio,4)} uncluster ratio\n')
          
        self.labels_ = self.scores[self.max]
        self.probabitilies_ = self.scores_prob[self.max]
