...
class COCO:
    ...
    def loadRes(self, resFile):
        ...
        """
        res = COCO() # Constructor of Microsoft COCO helper class for reading and visualizing annotations.
        ...
        time_t = datetime.datetime.utcnow()
        anns    = json.load(open(resFile)) # 加载模型预测结果
        assert type(anns) == list, 'results in not an array of objects'
        annsImgIds = [ann['image_id'] for ann in anns] # 提取image id
        ...
        if 'caption' in anns[0]:
            ...
            for id_, ann in enumerate(anns): # 增加索引
                ann['id'] = id_
