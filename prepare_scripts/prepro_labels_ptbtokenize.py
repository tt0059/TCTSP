...
def build_vocab(imgs, params):
  ...
  for i,img in enumerate(imgs):
    ...
    for sent in img['sentences']:
      cur_sent = re.sub('\.\.','.',sent['raw'])
      cur_sent = '. '.join(re.split('\.', cur_sent))
      tp_dict[i] = [{'caption': cur_sent}]
      #caption = tokenizer.tokenize(tp)[1][0].split()      
      #img['ptb_tokens'].append(caption)
  tokenized_sents = tokenizer.tokenize(tp_dict)
  for i,img in enumerate(imgs):
    img['ptb_tokens'] = tokenized_sents[i][0].split()
    debug = 1
...
if __name__ == "__main__":
  ...
  # input json
  parser.add_argument('--input_json', default = '../data_vg/para_filtered_v2_format.json', help='input json file to process into hdf5')
  parser.add_argument('--output_json', default='../data_vg/paratalk_filtered.json', help='output json file')
  parser.add_argument('--output_h5', default='../data_vg/paratalk_filtered', help='output h5 file')
