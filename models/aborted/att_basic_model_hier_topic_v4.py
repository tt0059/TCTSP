...
class AttBasicModel_HIER_TOPIC_V4(BasicModel):
    def __init__(self):
        super(AttBasicModel_HIER_TOPIC_V4, self).__init__()
        ...
        #use discrete 
        sequential = [nn.Embedding(se, cfg.MODEL.WORD_EMBED_DIM)]
        sequential.append(utils.activation(cfg.MODEL.WORD_EMBED_ACT))
        if cfg.MODEL.WORD_EMBED_NORM == True:
            sequential.append(nn.LayerNorm(cfg.MODEL.WORD_EMBED_DIM))
        if cfg.MODEL.DROPOUT_WORD_EMBED > 0:
                    ...
                    if cs_index[batch_id]==1:
                        if(self.training):
                            ...
                            if(sent_index[batch_id]<20):
                                ...
                                if(topic_seq_hard[batch_id, sent_index[batch_id]] != -2):
                                    new_wt_topic[batch_id] = topic_seq_hard[batch_id, sent_index[batch_id]].clone()
