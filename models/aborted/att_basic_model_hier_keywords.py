...
class AttBasicModel_HIER_KEYWORDS(BasicModel):
    def __init__(self):
        super(AttBasicModel_HIER_KEYWORDS, self).__init__()
    ...
    def make_kwargs(self, wt, gv_feat, att_feats, att_mask, p_att_feats, state, old_state, stacked_spatial,\
                    sent_index, keywords_embedding,  **kgs):
        kwargs = kgs
        ...
        kwargs[cfg.PARAM.OLD_STATE] = old_state
        kwargs[cfg.PARAM.STACKED_SPATIAL] = stacked_spatial
        kwargs[cfg.PARAM.SENT_INDEX] = sent_index
        kwargs[cfg.PARAM.KEYWORDS_LABELS] = keywords_embedding
    ...
    def preprocess(self, **kwargs):
        ...
        keywords_labels =  kwargs[cfg.PARAM.KEYWORDS_LABELS].long()
        ...
        #compute average keywords_embeddings  DETACHED! prevent them from training?
        batch_size = gv_feat.shape[0]
        keywords_mask = (keywords_labels !=0).float()
        keywords_embeddings = Variable(torch.zeros(batch_size, self.max_seq_num, cfg.MODEL.WORD_EMBED_DIM).cuda())#self.word_embed(keywords_labels).detach()
        
        for i in range(batch_size):
            for j in range(self.max_seq_num):
                cur_embedding = self.word_embed(keywords_labels[i][j]) #20*1024
                masked_cur_embedding = cur_embedding*keywords_mask[i][j].unsqueeze(1)
                avg_embedding = torch.sum(masked_cur_embedding)/torch.max(torch.tensor([torch.sum(keywords_mask[i][j]),1]))
                keywords_embeddings[i,j] = avg_embedding
        
        return gv_feat, att_feats, att_mask, p_att_feats, keywords_embeddings.detach()

    # gv_feat -- batch_size * cfg.MODEL.GVFEAT_DIM
    # att_feats -- batch_size * att_num * att_feats_dim
    def forward(self, **kwargs): 
        seq = kwargs[cfg.PARAM.INPUT_SENT] 
        gv_feat, att_feats, att_mask, p_att_feats, keywords_embeddings = self.preprocess(**kwargs)
        ...
        keywords_embedding = utils.expand_tensor(keywords_embeddings, cfg.DATA_LOADER.SEQ_PER_IMG) #bs*10*rnn_size
        ...
        old_state = self.word_embed(Variable(torch.ones(batch_size, dtype=torch.long).cuda())*(self.vocab_size-1))
        ...
        sent_index = Variable(torch.zeros(batch_size, 1).long().detach().cuda())
        
        acc_diff = Variable(torch.zeros(batch_size, 20, cfg.MODEL.RNN_SIZE).cuda())
        ...
        for t in range(seq.size(1)):
            ...
            cs_index = wt==2
            kwargs = self.make_kwargs(wt, gv_feat, att_feats, att_mask, p_att_feats, state, old_state, stacked_spatial, \
                                      sent_index, keywords_embedding)
            output, state, old_state, stacked_spatial, sent_index = self.Forward(**kwargs)
            
            
            for batch_id in range(batch_size):
                if cs_index[batch_id]==1:
                    if(self.training):
                        acc_diff[batch_id, sent_index-1] = keywords_embedding[batch_id,sent_index[batch_id]-1] - old_state[batch_id]
        ...
        return outputs, stacked_spatial, acc_diff

    def get_logprobs_state(self, **kwargs):
        output, state, old_state, stacked_alpha, sent_index = self.Forward(**kwargs)
        logprobs = F.log_softmax(self.logit(output), dim=1)
        return logprobs, state, old_state, stacked_alpha, sent_index
    ...
    def decode(self, **kwargs):
        ...
        gv_feat, att_feats, att_mask, p_att_feats, keywords_embedding = self.preprocess(**kwargs)
        batch_size = gv_feat.size(0)
        state = self.init_hidden(batch_size)
        ...
        old_state = self.word_embed(wt)
        stacked_spatial = Variable(torch.zeros(batch_size, 20, att_feats.shape[1] ).cuda()) #max seq num is 20
        sent_index = Variable(torch.zeros(batch_size, 1).long().detach().cuda())        
        for t in range(cfg.MODEL.SEQ_LEN):
            kwargs = self.make_kwargs(wt, gv_feat, att_feats, att_mask, p_att_feats, state, old_state, stacked_spatial, sent_index, keywords_embedding)
            logprobs_t, state, old_state, stacked_spatial, sent_index = self.get_logprobs_state(**kwargs)
