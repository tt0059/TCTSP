...
from models.att_basic_model_hier_keywords import AttBasicModel_HIER_KEYWORDS
import blocks

class XLAN_HIER_SUPKW(AttBasicModel_HIER_KEYWORDS):
    def __init__(self):
        super(XLAN_HIER_SUPKW, self).__init__()
    ...
    def Forward(self, **kwargs):
        ...
        #acc_diff = kwargs[cfg.PARAM.ACC_DIFF] 
        ...
        for batch_id in range(batch_size):
            if cs_index[batch_id]==1:
                old_state[batch_id] = output_topic[batch_id] #keywords_embedding[batch_id,sent_index[batch_id]]
                #####
                if(self.training):
        ...
        state = [torch.stack((h_att, output_topic, h_watt, output_word)), torch.stack((c_att, state[1][1], c_watt, state[1][3]))]
