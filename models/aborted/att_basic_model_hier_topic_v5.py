...
class AttBasicModel_HIER_TOPIC_V5(BasicModel):
    def __init__(self):
        super(AttBasicModel_HIER_TOPIC_V5, self).__init__()
        ...
        #use discrete 
        sequential = [nn.Embedding(1, cfg.MODEL.WORD_EMBED_DIM)]
        sequential.append(utils.activation(cfg.MODEL.WORD_EMBED_ACT))
        if cfg.MODEL.WORD_EMBED_NORM == True:
            sequential.append(nn.LayerNorm(cfg.MODEL.WORD_EMBED_DIM))
        if cfg.MODEL.DROPOUT_WORD_EMBED > 0:
    ...
    def make_kwargs(self, wt, topic_feats, gv_feat, att_feats, att_mask, p_att_feats, state_topic, state_word, old_state, **kgs):
        kwargs = kgs
    ...
    def forward(self, **kwargs): 
        ...
        topic_seq_hard = kwargs[cfg.PARAM.TOPIC_LABELS_HARD].long() 
        ...
        topic_feats = self.topic_embed(torch.zeros(batch_size).long().cuda().detach())
        old_state = self.topic_embed(torch.zeros(batch_size).long().cuda().detach())
        for t in range(seq.size(1)):
            ...
            kwargs= self.make_kwargs( wt, topic_feats, gv_feat, att_feats, att_mask, p_att_feats, state_topic, state_word, old_state)
            ...
            if(topic_renew == True):
                ...
                for batch_id in range(batch_size):
                    if cs_index[batch_id]==1:
                        ...
                        old_state[batch_id] = topic_feats[batch_id]
    ...
    def decode(self, **kwargs):
        ...
        for t in range(cfg.MODEL.SEQ_LEN):
            ...
            kwargs = self.make_kwargs(wt, topic_feats, gv_feat, att_feats, att_mask, p_att_feats, state_topic, state_word, old_state)
            logprobs_topic_t, state_topic, topic_feats = self.get_logprobs_state_topic(**kwargs)
            ...
            if(topic_renew == True):
                ...
                for batch_id in range(batch_size):
                    if cs_index[batch_id]==1:
                        ...
                        old_state[batch_id] = topic_feats[batch_id]
                        if(sent_index[batch_id]<20):
                            topic_logprobs[batch_id, sent_index[batch_id]] = logprobs_topic_t[batch_id, torch.argmax(logprobs_topic_t[batch_id]).clone()]
                            topic_sents[batch_id, sent_index[batch_id]] = torch.argmax(logprobs_topic_t[batch_id]).clone()                     
