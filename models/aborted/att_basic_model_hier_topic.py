...
class AttBasicModel_HIER_TOPIC(BasicModel):
    def __init__(self):
        super(AttBasicModel_HIER_TOPIC, self).__init__()
        self.ss_prob = 0.0                               # Schedule sampling probability
        self.vocab_size = cfg.MODEL.VOCAB_SIZE + 1       # include <BOS>/<EOS>
        self.topic_size = cfg.MODEL.TOPIC_SIZE #210 for original
        self.max_seq_num = 20
    ...
    def preprocess(self, **kwargs):
        ...
        topic_labels =  kwargs[cfg.PARAM.TOPIC_LABELS].long()
    ...
    def forward(self, **kwargs): 
        ...
        for t in range(seq.size(1)):
            ...
            kwargs = self.make_kwargs(wt, gv_feat, att_feats, att_mask, p_att_feats, state, old_state, stacked_spatial, \
                                      sent_index)
            output, state, old_state, stacked_spatial, sent_index, topic_logits = self.Forward(**kwargs)
            
            
            for batch_id in range(batch_size):
                if cs_index[batch_id]==1:
                    if(self.training):
                        topic_outputs[batch_id, sent_index[batch_id]-1] = topic_logits[batch_id]
    ...
    def get_logprobs_state(self, **kwargs):
        output, state, old_state, stacked_alpha, sent_index, topic_logits = self.Forward(**kwargs)
        logprobs = F.log_softmax(self.logit(output), dim=1)
        return logprobs, state, old_state, stacked_alpha, sent_index, topic_logits
    ...
    def decode(self, **kwargs):
        ...
        for t in range(cfg.MODEL.SEQ_LEN):
            cs_index = wt==2
            kwargs = self.make_kwargs(wt, gv_feat, att_feats, att_mask, p_att_feats, state, old_state, stacked_spatial, sent_index)
            logprobs_t, state, old_state, stacked_spatial, sent_index, topic_logits = self.get_logprobs_state(**kwargs)
            
            if(True or self.training):
                for batch_id in range(batch_size):
                    if cs_index[batch_id]==1:
                        if(sent_index[batch_id]<21):
                            if(greedy_decode == False):
                                topic_logP_t, topic_wt = torch.max(F.log_softmax(topic_logits,-1), 1)
                                topic_sents[batch_id, sent_index[batch_id]-1] = topic_wt[batch_id]
                                topic_logprobs[batch_id, sent_index[batch_id]-1] = topic_logP_t[batch_id]
                            else:
                                topic_sents[batch_id, sent_index[batch_id]-1] = torch.argmax(topic_logits[batch_id])
