...
class AttBasicModel_HIER_TOPIC_V6_XE_topic(BasicModel):
    def __init__(self):
        super(AttBasicModel_HIER_TOPIC_V6_XE_topic, self).__init__()
    ...
    def preprocess(self, **kwargs):
        ...
        # bilinear
        if cfg.MODEL.BILINEAR.DIM > 0:
            gv_feat, att_feats = self.encoder_layers(gv_feat, att_feats, att_mask) # X-LAN Encoder
    ...
    def forward(self, **kwargs): # 总前传
        #torch.autograd.set_detect_anomaly(True)
        seq = kwargs[cfg.PARAM.INPUT_SENT] 
        topic_seq = kwargs[cfg.PARAM.TOPIC_LABELS]#.long() 软标签
        gv_feat, att_feats, att_mask, p_att_feats = self.preprocess(**kwargs) # X-LAN Encoder
        ...
        outputs = Variable(torch.zeros(batch_size, seq.size(1), self.vocab_size).cuda()) # 图片的段落描述单词预测结果
        stacked_spatial = Variable(torch.zeros(batch_size, 20, att_feats.shape[1]).cuda()) # max seq num is 20
        topic_kls = Variable(torch.zeros(batch_size, 20).cuda()) # 每句话的topic预测的kl散度损失
        ...
        for t in range(seq.size(1)):
            ...
            if(t == 0):
                wt_topic = (self.topic_size - 1) * torch.ones(batch_size).long().cuda().detach() # topic bos
            cs_index = (wt==2) + (t == 0)     ##  wt==2表明一个句子结束（但是最后一句话不用2做结尾），t==0表明一个段落开始  cs_index([20])    
            topic_renew = False               ##  topic更新信号
            for batch_id in range(batch_size):
                if(cs_index[batch_id]==1):
                    topic_renew = True        ##  只要有一个段落需要新预测topic，就设置topic更新信号为True
                    break             
            #forward topic LSTM only when an sentence ends
            if(topic_renew == True):
                kwargs= self.make_kwargs(wt_topic, wt, topic_feats, gv_feat, att_feats, att_mask, p_att_feats, state_topic, state_word) # wt、gv_feat、att_feat、att_mask、p_att_feats是真实标注
                topic_feats_new, state_topic_new = self.Forward_Topic(**kwargs) # 整个batch做topic预测
                ...
                if self.dropout_lm is not None:
                    topic_logits = self.topic_logit(self.dropout_lm(topic_feats_new)) # 线性层将特征转化为topic——size形状
                else:
                ...
                for batch_id in range(batch_size): # 在当前batch中每句话进行搜索
                    if cs_index[batch_id]==1: # 只有需要的语句才会对topic_feats、state_topic
                        ...
                        if(sent_index[batch_id]<20):
                            kl_div = F.cross_entropy(topic_logits[batch_id].unsqueeze(0), torch.tensor([torch.argmax(topic_seq[batch_id, sent_index[batch_id]])]).cuda(), reduction='sum') 
                            topic_kls[batch_id, sent_index[batch_id]] = kl_div   # 存储kl散度损失,每段话有一个列表，连续排列了每一句的kl散度，填充为 0.00            
                            new_wt_topic[batch_id] = torch.argmax(topic_seq[batch_id, sent_index[batch_id]]) # 将topic真实标注硬标签作为下一次的输入 
                        #also init the state of the words_lstm
                        if(t != 0): # 若不是预测第0个词，说明是某句话结束了，要开始新的一句话，这是从判断条件cs_index = (wt==2) + (t == 0) 得出的
                            new_wt[batch_id] = seq[batch_id, 0].clone() # 下一次要输入的是<BOS>
                            state_word[0][0][batch_id,:] = 0
                            state_word[0][1][batch_id,:] = 0 # 
                            #state_word[1][0][batch_id,:] = 0
                            state_word[1][1][batch_id,:] = 0                            
                state_topic[0] = state_topic_tp0 # 更新state_topic
                state_topic[1] = state_topic_tp1
                wt_topic = new_wt_topic.clone() # 真实标注更新
                wt = new_wt.clone() # 真实标注更新
            ...
            outputs[:, t] = logit # 存储该batch所有句子的所有单词的概率分布还没softmax
            #outputs_attn[:, t] = alpha_spatial

        return outputs, stacked_spatial, topic_kls # stacked_spatial没用到貌似
    def decode(self, **kwargs):
        greedy_decode = kwargs['GREEDY_DECODE'] # 
        block_trigrams = not self.training
        trigrams = [] # will be a list of batch_size dictionaries
        gv_feat, att_feats, att_mask, p_att_feats = self.preprocess(**kwargs) # 编码器
        batch_size = gv_feat.size(0)
        state_topic, state_word = self.init_hidden(batch_size) # 初始化状态
        #old_state = gv_feat.new_zeros(batch_size, cfg.MODEL.RNN_SIZE)
       
        sents = Variable(torch.zeros((batch_size, cfg.MODEL.SEQ_LEN), dtype=torch.long).cuda()) # 段落存储变量
        ...
        unfinished = wt.eq(wt) # 设置了一个与batch——size等大的bool tensor
        ...
        for t in range(cfg.MODEL.SEQ_LEN):  # 总体由单词数来控制循环数       
            cs_index = (wt==2) + (t == 0)   
            topic_renew = False   
            for batch_id in range(batch_size):
                if(cs_index[batch_id]==1):
            ...
            if(topic_renew == True): # 如果存在某段要更新topic
                kwargs= self.make_kwargs(wt_topic, wt, topic_feats, gv_feat, att_feats, att_mask, p_att_feats, state_topic, state_word) 
                ...
                for batch_id in range(batch_size): # 
                    if cs_index[batch_id]==1: # 
                        sent_index[batch_id] += 1 # 
                        topic_feats[batch_id] = topic_feats_new[batch_id] # 更新该段落topic feat
                        state_topic_tp0[:, batch_id] = state_topic_new[0][:, batch_id] # = state_topic_new[0][:, batch_id] 更新状态
                        state_topic_tp1[:, batch_id] = state_topic_new[1][:, batch_id]
                        
                        new_wt_topic[batch_id] = torch.argmax(logprobs_topic_t[batch_id]).clone() # 使用预测出的topic更新下一次输入给topic decoder的wt_topic,当然是更新需要更新的段落
                        if(sent_index[batch_id]<20):
                            topic_logprobs[batch_id, sent_index[batch_id]] = logprobs_topic_t[batch_id, new_wt_topic[batch_id]] # 存储该段落该句话的topic单独的预测logprobs
                            topic_sents[batch_id, sent_index[batch_id]] = new_wt_topic[batch_id]     # 存储该段落该句话的topic 
                        #also init the state of the words_lstm
                        if(t != 0): # 
                            new_wt[batch_id] = self.vocab_size-1#seq[batch_id, 0].clone() # 重新设置输入的wt
                            state_word[0][0][batch_id,:] = 0
                            state_word[0][1][batch_id,:] = 0
                            #state_word[1][0][batch_id,:] = 0 # 保留该状态做后续使用
                            state_word[1][1][batch_id,:] = 0                           
                state_topic[0] = state_topic_tp0 # 更新状态
                state_topic[1] = state_topic_tp1
                wt_topic = new_wt_topic.clone() # 更新下一次预测的wt_topic
                wt = new_wt.clone() # 更新wt
                                
                        
            kwargs = self.make_kwargs(wt_topic, wt, topic_feats, gv_feat, att_feats, att_mask, p_att_feats, state_topic, state_word)
            logprobs_t, state_word = self.get_logprobs_state_word(**kwargs)  # 开始预测该句话，logprobs_t就是logprobs_word
            # Mess with trigrams
            ...
            else: # 从这里开始，跳过了（if block_trigrams and t >= 3 and greedy_decode:）和（if greedy_decode:）
                probs_t = torch.exp(logprobs_t) # probs_word
                wt = torch.multinomial(probs_t, 1) # 根据概率对每一个段落当前预测单词进行采样
                logP_t = logprobs_t.gather(1, wt) # 获取采样单词所对应的logprobs
                
            wt = wt.view(-1).long() # 将列向量转变为行向量
            unfinished = unfinished * (wt > 0) 
            wt = wt * unfinished.type_as(wt) 
            sents[:,t] = wt # 存储decode出来的句子
            logprobs[:,t] = logP_t.view(-1) # 存储预测出的单词的logprobs
       
            if unfinished.sum() == 0: # 如果所有的段落都已预测结束，那么则终止预测
                break
            
        return sents, logprobs, topic_sents, topic_logprobs # 返回预测出的句子，预测出的单词的logprobs，预测出的topic序列、预测出的topic单独的logprobs

    def get_logprobs_state_topic(self, **kwargs): # 这个函数只是在Forward_Topic的基础上封装了logprobs_topic并返回
    ...
    def get_logprobs_state_word(self, **kwargs): # 这个函数只是在Forward_Word的基础上封装了logprobs_word并返回
        output_word, state_word = self.Forward_Word(**kwargs)
