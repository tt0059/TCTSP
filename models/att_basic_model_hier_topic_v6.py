...
class AttBasicModel_HIER_TOPIC_V6(BasicModel):
    def __init__(self):
        super(AttBasicModel_HIER_TOPIC_V6, self).__init__()
    ...
    def forward(self, **kwargs): 
        ...
        topic_seq = kwargs[cfg.PARAM.TOPIC_LABELS]
                ...
                for batch_id in range(batch_size): # 在当前batch中每句话进行搜索
                    if cs_index[batch_id]==1: # 只有需要的语句才会对topic_feats、state_topic
                        ...
                        if(sent_index[batch_id]<20):
                            kl_div = F.kl_div(torch.log_softmax(topic_logits[batch_id], dim=0), topic_seq[batch_id, sent_index[batch_id]], reduction='sum') 
    ...
    def decode(self, **kwargs):
        ...
        for t in range(cfg.MODEL.SEQ_LEN):  # 总体由单词数来控制循环数       
            ...
            if(topic_renew == True): # 如果存在某段要更新topic
                kwargs= self.make_kwargs(wt_topic, wt, topic_feats, gv_feat, att_feats, att_mask, p_att_feats, state_topic, state_word) 
                logprobs_topic_t, state_topic_new, topic_feats_new = self.get_logprobs_state_topic(**kwargs) # 返回整个batch的logprobs_topic\state_topic\topic_feats，但是不会全部使用，除非段落开头
                ...
                state_topic_tp0 = state_topic[0].clone()
                state_topic_tp1 = state_topic[1].clone() 
                for batch_id in range(batch_size): # 在整个batch中搜索哪些段落需要更新topic
                    if cs_index[batch_id]==1: # 如果需要更新
                        sent_index[batch_id] += 1 # 当前段落生成的句子数加1
