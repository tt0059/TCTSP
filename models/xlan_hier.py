...
from models.att_basic_model_hier import AttBasicModel_HIER
import blocks

class XLAN_HIER(AttBasicModel_HIER):
    def __init__(self):
        super(XLAN_HIER, self).__init__()
    ...
    def Forward(self, **kwargs):
        ...
        output_topic = self.att2ctx(ctx_input)
             
        
        ######word level########
        batch_size = gv_feat.size(0)
        for batch_id in range(batch_size):
            if cs_index[batch_id]==1: # 如果一句话结束 
                old_state[batch_id] = output_topic[batch_id]
