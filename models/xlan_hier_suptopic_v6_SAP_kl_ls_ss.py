...
from models.att_basic_model_hier_topic_v6_kl_ls_ss import AttBasicModel_HIER_TOPIC_V6_kl_ls_ss
import blocks
...
class XLAN_HIER_SUPTOPIC_V6_SAP_kl_ls_ss(AttBasicModel_HIER_TOPIC_V6_kl_ls_ss):
    def __init__(self,arg):
        super(XLAN_HIER_SUPTOPIC_V6_SAP_kl_ls_ss, self).__init__()
        self.arg = arg
    ...
    def Forward_Topic(self, **kwargs):
        ...
        sap_logits = self.SAP(self.topic_embed(torch.Tensor([range(self.topic_num)]).long().cuda()).detach().squeeze(0),h_att,wt_topic,self.subsequent_mat) #!! 输入接好后需要看一下是否真的起到作用
