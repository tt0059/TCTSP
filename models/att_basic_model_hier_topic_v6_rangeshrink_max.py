...
from losses.RangeShrink_max import RangeShrink_max

class AttBasicModel_HIER_TOPIC_V6_rangeshrink_max(BasicModel):
    def __init__(self):
        super(AttBasicModel_HIER_TOPIC_V6_rangeshrink_max, self).__init__()
        ...
        self.topic_criterion = RangeShrink_max()
        # bilinear
    ...
    def forward(self, **kwargs): 
        ...
        stacked_spatial = Variable(torch.zeros(batch_size, 20, att_feats.shape[1]).cuda()) 
        topic_RangeShrink = Variable(torch.zeros(batch_size, 20).cuda()) 
        sent_index = - torch.ones(batch_size).long().detach().cuda() 
                ...
                for batch_id in range(batch_size): 
                    if cs_index[batch_id]==1: 
                        ...
                        if(sent_index[batch_id]<20):
                            ...
                            topic_RangeShrink[batch_id, sent_index[batch_id]] = kl_div                     
        ...
        return outputs, stacked_spatial, topic_RangeShrink # 
            ...
            if(topic_renew == True): # 
                ...
                for batch_id in range(batch_size): # 
                    if cs_index[batch_id]==1: # 
                        sent_index[batch_id] += 1 # 
                        topic_feats[batch_id] = topic_feats_new[batch_id] # 
                        state_topic_tp0[:, batch_id] = state_topic_new[0][:, batch_id] #
                        state_topic_tp1[:, batch_id] = state_topic_new[1][:, batch_id]
                        ...
                        #also init the state of the words_lstm
                        if(t != 0): # 
                            new_wt[batch_id] = self.vocab_size-1#
                            state_word[0][0][batch_id,:] = 0
                            state_word[0][1][batch_id,:] = 0
                            #state_word[1][0][batch_id,:] = 0 # 
                            state_word[1][1][batch_id,:] = 0                           
