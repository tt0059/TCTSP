...
class AttBasicModel(BasicModel):
    ...
    def forward(self, **kwargs): 
        ...
        outputs_attn = Variable(torch.zeros(batch_size, seq.size(1), att_feats.shape[1] ).cuda())
        for t in range(seq.size(1)):
            ...
            output, state, old_att, alpha_spatial = self.Forward(**kwargs)
        ...
        return outputs, outputs_attn

    def get_logprobs_state(self, **kwargs):
        output, state, alpha_spatial = self.Forward(**kwargs)
        logprobs = F.log_softmax(self.logit(output), dim=1)
        return logprobs, state, alpha_spatial
