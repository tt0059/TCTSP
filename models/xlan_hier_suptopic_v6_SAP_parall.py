...
from models.att_basic_model_hier_topic_v6_SAP_PARA import AttBasicModel_HIER_TOPIC_V6_SAP_PARA
import blocks
...
class subsequent_attribute_predictor(nn.Module):
    def __init__(self,opt):
        ...
        self.f2logit = nn.Linear(2*self.rnn_size, self.opt.topic_num-1) # 由 att-lstm以及图卷积两个输出作为输入，之后输出维度等同于att-lstm输出维度
    ...
    def forward(self,word_embedding,h_att,previous_attr,subsequent_mat):
        ...
        x3 = torch.mm(subsequent_mat,x2)              #  } GCN Layers?            3.subsequent_mat: 转移概率矩阵
        x4 = self.attr_fc2(x3)                        # /
        x5 = self.attr_fc3(x4)                        #/
...
class XLAN_HIER_SUPTOPIC_V6_SAP_parall(AttBasicModel_HIER_TOPIC_V6_SAP_PARA):
    def __init__(self,arg):
        super(XLAN_HIER_SUPTOPIC_V6_SAP_parall, self).__init__()
        self.arg = arg
    ...
    def Forward_Topic(self, **kwargs):
        ...
        return topic_feats, state_topic, sap_logits
