...
class LowRankBilinearLayer(nn.Module):
    ...
    def forward(
        ...
        x, alpha_spatial = self.encoder_attn(
            query=x,
...
class LowRankBilinearEncBlock(nn.Module):
    ...
    def forward(self, gv_feat, att_feats, att_mask, p_att_feats=None): # X-LAN Encoder
        if gv_feat == None or gv_feat.shape[-1] == 1:  # empty gv_feat
            if(att_mask is not None):
                gv_feat = torch.sum(att_feats * att_mask.unsqueeze(-1), 1) / torch.sum(att_mask.unsqueeze(-1), 1) # gv_feat为非填充att_feats的平均值
            else:
                gv_feat = torch.mean(att_feats,dim=1)
        
        feat_arr = [gv_feat]
        for i, layer in enumerate(self.layers):
            gv_feat, _ = layer(gv_feat, att_feats, att_mask, gv_feat, att_feats)
...
class LowRankBilinearDecBlock(nn.Module):
    ...
    def forward(self, gv_feat, att_feats, att_mask, p_att_feats=None, precompute=False):
        ...
        for i, layer in enumerate(self.layers):
            ...
            gv_feat, alpha_spatial = layer(gv_feat, key, att_mask, gv_feat, value2, precompute)
            feat_arr.append(gv_feat)

        gv_feat = torch.cat(feat_arr, dim=-1)
        gv_feat = self.proj(gv_feat)
        gv_feat = self.layer_norm(gv_feat)
        return gv_feat, att_feats, alpha_spatial # 参数不重要
